...
/// <summary>
/// 一定時間で線形変化する値。 かなりの量のEasingに対応しました。
/// </summary>
[System.Serializable]
public class LinearValue
{
	...
	// switch case が長くなってしまうので、関数テーブルにしました。
	static Dictionary<EaseType, EasingFunction> funcTable = new Dictionary<EaseType, EasingFunction>()
	{
	...
	public void Init(float time, float valueBegin, float valueEnd, EaseType easeType = EaseType.easeOutQuad)
	{
		this.time = time;
		this.timeCurrent = 0;
	...
	public void IncTime()
	{
		...
		valueCurrent = easeFunc(valueBegin, valueEnd, timeCurrent / time);
	}
